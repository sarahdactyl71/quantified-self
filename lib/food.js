...
Food.editFoodName = function () {
  ...
  const foodID = this.parentElement.attributes[1].value
  const foodCalories = this.parentElement.children[1].innerText
  const $input = $('<input/>').val( $originalValue.text () )
  $originalValue.replaceWith ( $input )

  const save = function () {
    const foodName = $input.val ()
    Food.updateFood (foodID, foodName, foodCalories)
    const $editedValue = $(`<td class="food-name id-${foodID}" >${$input.val ()}</td>`)
...
Food.editFoodCalories = function () {
  const $originalValue = $(this)
  const foodID = this.parentElement.attributes[1].value
  const foodName = this.parentElement.children[0].innerText
  const $input = $('<input/>').val( $originalValue.text () )
  $originalValue.replaceWith ( $input )

  const save = function () {
    const foodCalories = $input.val ()
    Food.updateFood (foodID, foodName, foodCalories)
    const $editedValue = $(`<td class="food-calories id-${foodID}" >${$input.val ()}</td>`)
    $input.replaceWith ( $editedValue )
  }
