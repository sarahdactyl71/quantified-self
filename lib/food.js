...
Food.editFood = function (data) {
  var $originalValue = $(data.tableData)
  var foodID = data.tableData.parentElement.attributes[1].value
  // $(`tr[data-id='${data.tableData.attributes["data-id"].value}']`)
  var foodUnchanged = data.tableData.parentElement.children[data.dataIndex].innerText
  var $input = $('<input/>').val( $originalValue.text () )
  $originalValue.replaceWith ( $input )

  var save = function () {
    var foodChanged = $input.val ()
    var updateAttrs = {
      foodID,
    ...
    var $editedValue = $(`<td class="food-${data.changed} id-${foodID}" >${$input.val ()}</td>`)
