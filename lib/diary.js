...
Diary.calculateCalories = function () {
  var calorieCounts = $('.breakfast-calories-total').map(function () {
	    return parseInt($(this).text())
    }).toArray()
  var summedCalories = 0
  calorieCounts.forEach(function(calorieCount) {
    summedCalories += calorieCount
    Diary.appendCaloriesTotal(summedCalories)
  })
}

Diary.appendCaloriesTotal = function (total) {
  $('.breakfast-total-calories').empty ()
  $('.breakfast-total-calories').append (total)
}
